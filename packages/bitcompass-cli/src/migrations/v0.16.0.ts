...
export const migration: Migration = {
  version: '0.16.0',
  scope: 'project',
  description: 'Migrate skill files to subdirectories and relocate special files',

  async migrate(opts?: { dryRun?: boolean }): Promise<void> {
    ...
    // Idempotency: skip if already at or beyond this version
    if (
      config.configVersion !== undefined &&
      typeof config.configVersion === 'string' &&
      compareVersion(config.configVersion, '0.16.0') >= 0
    ) {
      return;
    }
